...
if ($conn->query($sql_end) === TRUE) {
    ...
    if ($conn->query($sql_score) === TRUE) {
        ...
        if ($conn->query($sql_staging) === TRUE) {
            $sql_competitive = "INSERT INTO Competitive (StagingID, CompetitionID, DivisionID) VALUES ('$archer_id', '$competition_id', '$division_id')";
            if ($conn->query($sql_competitive) === TRUE) {
                echo "Data successfully inserted!";
            } else {
                echo "Error: " . $sql_competitive . "<br>" . $conn->error;
            }
        } else {
